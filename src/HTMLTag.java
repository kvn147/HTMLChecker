...
// as HTMlManager.java, HTMLTag.java, HTMLTagType.java HTMLChecker.java
...
public class HTMLTag {
    ...
    private static final Set<String> SELF_CLOSING_TAGS = new HashSet<>(Arrays.asList("!DOCTYPE", "area", "base", "basefont",
    ...
    /**
     * Returns true if this HTML tag is a "closing" (finishing) tag.
    ...
    /**
     * Returns true if this tag does not require a matching closing tag,
    ...
    public boolean isComment() {
        return this.element.trim().startsWith("--");
    }
    ...
    public boolean equals(HTMLTag other) {
        return this.element.trim().equalsIgnoreCase(other.element.trim()) && this.type == other.type;
    }
    ...
    public String toString() {
        if (this.isComment()) {
            ...
            return "<" + this.element.trim() + attr + ">" + contents;
        } else if (this.type == HTMLTagType.CLOSING) {
            String contents = this.contents == null ? "" : this.contents;
            return "</" + this.element.trim() + ">" + contents;
        } else if (this.type == HTMLTagType.SELF_CLOSING) {
            String attr = (this.attributes.isEmpty() ? " " + this.attributes : "");
            return "<" + this.element.trim() + attr + "/>" + contents;
        } else {
            System.err.println("Invalid Tag Type.");
            System.exit(1);
